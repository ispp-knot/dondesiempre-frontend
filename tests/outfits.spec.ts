...
test.describe.serial('outfits', () => {
    ...
    test('create outfit', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Añadir imagen' }).setInputFiles('./scripts/promocion.jpg');
    ...
    test('edit outfit, only info', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Preview Cambiar imagen' }).setInputFiles('./scripts/Foto Mapa completo muchos comercios.png');
        await page.getByRole('spinbutton', { name: 'Descuento' }).click();
    ...
    test('sort outfits', async ({ page }) => {
        ...
        await page.locator('div').filter({ hasText: /^Crear outfit$/ }).click();
        await page.waitForURL(/stores\/.*\/create-outfit/);
